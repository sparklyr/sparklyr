...
For YARN Cluster Mode, we should probably have a connection example and also mention that they need to correctly configured the client machine for YARN Cluster mode, maybe add a reference back to the official docs and mention that at the very least they need to copy: `yarn-site.xml` and `hive-site.xml`. The trick here is that there might be other files they need to configure for their particular cluster so we can’t be too prescriptive.
