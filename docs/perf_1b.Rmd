...
spark_sum_range_parquet <- function(sc, db, ses, df) {
  df <- invoke(sparklyr:::spark_sql_or_hive(sparklyr:::spark_api(db)), "read") %>%
    invoke("parquet", list(parquetPath))
    
  result <- invoke(df, "selectExpr", list("sum(x)")) %>%
    invoke("collect")
