...
stream_register <- function(stream) {
  ...
  if (
    ...
    jobActions <- list(
      info = function(id) {
        named_sc <- Filter(
          function(e) identical(get(e, envir = .GlobalEnv), sc),
          ls(envir = .GlobalEnv)
        )[[1]]
        if (nchar(named_sc) == 0) {
          named_sc <- paste0(
...
stream_unregister <- function(stream) {
  ...
  if (!is.null(sc$state$streams)) {
    ...
    if (!is.null(stream_ref)) {
      if (!is.null(stream_ref$job)) {
        rstudio_jobs_api()$remove_job(stream_ref$job)
      }
...
stream_stop <- function(stream) {
  if (!is.null(stream$job)) {
    rstudio_jobs_api()$add_job_progress(stream$job, 100L)
  }
...
stream_validate <- function(stream) {
  ...
  while (
    !grepl("waiting", stream_status(stream)$message) &&
...
stream_trigger_interval <- function(
  ...
  structure(
    class = c("stream_trigger_interval"),
    list(
      interval = interval
    )
  )
...
stream_trigger_continuous <- function(
  ...
  structure(
    class = c("stream_trigger_continuous"),
    list(
      interval = checkpoint
    )
  )
...
stream_generate_test_entry <- function(
  ...
  if (idxDist > length(distribution)) {
    idxDist <- 1
  }
  ...
  write.csv(
    rows,
    file.path(path, paste0("stream_", idxFile, ".csv")),
    row.names = FALSE
  )
  Sys.sleep(interval)
