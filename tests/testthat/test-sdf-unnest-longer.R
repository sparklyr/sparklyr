skip_connection("sdf-unnest-longer")
skip_on_livy()
skip_on_arrow_devel()
...
test_that("automatically adds id col if named", {
  ...
  expect_equivalent(
    sdf %>% sdf_unnest_longer(y) %>% collect(),
    dplyr::tibble(x = c(1L, 1L, 2L, 2L), y = 1:4, y_id = rep(c("a", "b"), 2))
  )
})
...
test_that("`include_indices = FALSE` works as expected", {
  ...
  expect_equivalent(
    sdf %>% sdf_unnest_longer(y, include_indices = FALSE) %>% collect(),
    dplyr::tibble(x = c(1L, 1L, 2L, 2L), y = 1:4)
  )
})
...
test_that("`names_repair` works as expected", {
  ...
  sdf <- copy_to(
    sc,
    dplyr::tibble(y_id = 1:3, y = list(1:10, 1:5, 1:2))
  )
