...
#' @export
pivot_wider.tbl_spark <- function(
  data,
  id_cols = NULL,
  id_expand = FALSE,
...
.process_value_spec <- function(
  ...
  all_obvs <- lhs %>%
    dplyr::ungroup() %>>%
    dplyr::distinct %@%
    lapply(union(seq_col, key_vars), as.symbol)
...
.summarize_data <- function(data, values_fn, key_vars, names_from) {
  ...
  for (idx in seq_along(values_fn)) {
    ...
    summarizers[[col]] <- (if (is.null(values_fn[[idx]])) {
  ...
  data %>>%
    dplyr::group_by %@%
    lapply(union(key_vars, names_from), as.symbol) %>>%
    dplyr::summarize %@%
    summarizers
}
...
.apply_pivot_wider_names_repair <- function(
  ...
  names(name_repair_args) <- head(
    output_colnames,
    length(key_vars) + length(other_cols)
  )
