      ...
      spark_perf_test(
        params = list(
          list(
            ...
            suffle = optimizeShuffleForCores
          )
        ),
        tests = list(
...
results <- results %>% 
  mutate(params = paste(run, version, cores, cache, logging, partitions, suffle))
```
...
results %>%
  ...
  dcast(run + cores + version + logging + part + suffle ~ test, value.var = "elapsed")
```
