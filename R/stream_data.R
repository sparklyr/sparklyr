                                ...
                                stream_options)
{
  ...
  tbl(sc, name) %>%
    mutate(timestamp = current_timestamp())
}

stream_write_generic <- function(x,
                                 path,
                                 type,
                                 mode,
                                 trigger,
...
#' @param watermark Time in seconds before which to assume no more late data is
#' going to arrive.
#'
#' @family Spark stream serialization
#'
