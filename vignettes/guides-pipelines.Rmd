...
The `parm_map` attribute contains the SQL statement that will be used for the pipeline.  Notice that the `flights` table name has been replace with `__THIS__`.  This allows the pipeline to accept different table names as its source, making the pipeline very modular. 

```{r}
ft_dplyr_transformer(sc, df)$param_map
```
