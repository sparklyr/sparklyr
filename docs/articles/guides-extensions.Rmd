...
|----------------------------|---------------------------------------------|
| [`spark_connection`](reference/sparklyr/latest/spark_connection.html) | Connection between R and the Spark shell process |
...
| Function | Description  |
|----------------------------|---------------------------------------------|
| [`spark_connection`](reference/sparklyr/latest/spark_connection.html) | Get the Spark connection associated with an object (S3)|
...
| [`spark_context`](reference/sparklyr/latest/spark_context.html) | Get the SparkContext for a `spark_connection` |
| [`hive_context`](reference/sparklyr/latest/hive_context.html) | Get the HiveContext for a `spark_connection` |
...
|             | Array[AnyVal]    | vector    |
|             | Array[Date]      | Date      |
|             | Array[Timestamp] | POSIXct   |
|             | Array[Object]    | list      |
| environment | Map[String, T]   |           |
| jobj        | Object           | jobj      |

Notice that [AnyVal](https://www.scala-lang.org/api/current/scala/AnyVal.html) includes
`Double`, `Float`, `Long`, `Int`, `Char`, `Short`, `Byte` and `Boolean` as subtypes.
