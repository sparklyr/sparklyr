...
spark_perf_test <- function(params, tests) {
  resultsList <- lapply(params, function(param) {
    ...
    config <- spark_config()
    config[["sparklyr.sql.shuffle.partitions"]] <- if(param$shuffle) parallel::detectCores() else NULL
    config[["spark.sql.codegen.wholeStage"]] <- param$codegen
    config[["sparklyr.cores"]] <- param$cores
    
    sc <- spark_connect(master = param$master, version = param$version, config = testConfig)
